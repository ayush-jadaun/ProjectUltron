...
const getPageGradients = (pathname) => {
	switch (pathname) {
		...
		case "/analysis":
			return {
				bg: "bg-gradient-to-r from-indigo-700 to-indigo-500",
...
const Footer = () => {
	...
	const theme = getPageGradients(location.pathname);

	return (
		<footer className={`${theme.bg} ${theme.text} py-8 relative shadow-lg`}>
			{/* Decorative top border */}
			<div
				className={`absolute top-0 left-0 right-0 h-1 ${theme.border}`}
			></div>

			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="flex flex-col items-center text-center">
					<h2 className="text-2xl font-bold tracking-tight">Project Ultron</h2>
				...
				<div className="mt-4 pt-4 border-t border-white border-opacity-20 text-center text-sm opacity-70">
					© {new Date().getFullYear()} Project Ultron. All rights reserved.
				</div>
			</div>
		</footer>
	);
